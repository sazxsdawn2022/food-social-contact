...
public class SignService {
    ...
    public Map<String, Boolean> getSignInfo(String accessToken, String dateStr) {
        ...
        if (list == null || list.isEmpty()) {
            return signInfo; // 如果为空，返回的就是空map
        }
    ...
    /**
     * 统计连续签到的次数
     * 传入的当天对应的位域的位往左数连续为1的值，即为连续签到的次数
     *
     * @param dinerId
     * @param date
     * @return 截至到当前天，连续签到的次数
     */
    private int getContinuousSignCount(Integer dinerId, Date date) {
        ...
        // bitfield user:sgin:5:202011 u30 0  30是位域的偏移量，对应dayOfMonth，即计算当天之前连续签到的天数
        BitFieldSubCommands bitFieldSubCommands = BitFieldSubCommands.create()
                .get(BitFieldSubCommands.BitFieldType.unsigned(dayOfMonth))
                .valueAt(0);
        List<Long> list = redisTemplate.opsForValue().bitField(signKey, bitFieldSubCommands);
        ...
        int signCount = 0;
        long v = list.get(0) == null ? 0 : list.get(0); // get是bitfield命令的子命令，0代表从位域的第0位开始获取值
