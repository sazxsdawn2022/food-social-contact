 ...
 */
@Configuration
@EnableAuthorizationServer      //继承Spring提供的一个适配器
public class AuthorizationServerConfiguration extends AuthorizationServerConfigurerAdapter {
    ...
    /**
     * 配置令牌端点安全约束
     * Security环境搭好之后，内部有的一些端点，这些请求是受保护的，默认放行
     *
     * @param security
     * @throws Exception
     */
    ...
    public void configure(AuthorizationServerSecurityConfigurer security) throws Exception {
        // 允许访问 token 的公钥，不写的话默认 /oauth/token_key 是受保护的
    ...
    /**
     * 客户端配置 - 授权模型，就是application.yml文件中配置的
     * 我们用的Password
     * 这里配置的实际上就是 Oauth2中client的一些东西
     * 存到内存中去，这里是写死了
     * @param clients
     * @throws Exception
     */
    ...
    public void configure(AuthorizationServerEndpointsConfigurer endpoints) throws Exception {
        ...
        endpoints.authenticationManager(authenticationManager)
                //写一个登录逻辑
                // 具体登录的方法自己写，传入用户然后校验然后生成令牌token 是Security OAuth2帮助完成的
                .userDetailsService(userService)  //自己写的userService
                // token 存储的方式：Redis
